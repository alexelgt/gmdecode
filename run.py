...
parser.add_argument("-g", "--gamemaster_filename", help="Game Master file name")
    ...
    try:
        ...
        try:
            from pyproto.gamemaster_pb2 import (
                DownloadGmTemplatesResponseProto,  # pylint: disable=import-outside-toplevel, no-name-in-module
            )

            gamemaster_txt = read_txt_file(gamemaster_output_file)

            gamemaster_txt_parsed = text_format.Parse(
                gamemaster_txt, DownloadGmTemplatesResponseProto(), allow_unknown_field=True
            )

            gamemaster_json_string = json_format.MessageToJson(gamemaster_txt_parsed)
                    ...
                    for blocks_meta_element in blocks_meta_info:
                        try:
                            if blocks_meta_element["regex_find"] is not None:
                                ...
                                is_template_id = (
                                    blocks_meta_element["string_find"] in element["templateId"]
                                )
                            else:
                                is_template_id = False

                            if blocks_meta_element["block_name"] == "POKEMON_ID_BLOCK" and (
                                "form"
                                in element["data"][blocks_meta_element["element_first_level"]]
                            ):
                                continue

                            if is_template_id and isinstance(
                                element["data"][blocks_meta_element["element_first_level"]][
                                ...
                                if blocks_meta_element["enum_key_default_value"]:
                                    ...
                                    _, enum_key = (
                                        blocks_meta_element["regex_find"]
                                        .search(element["templateId"])
                                        .groups()
                                    )
                                else:
                                ...
                                if len(blocks_meta_element["enum_key_replace"]) == 2:
                                    enum_key.replace(*blocks_meta_element["enum_key_replace"])

                                enum_value = element["data"][
                                    blocks_meta_element["element_first_level"]
                                ][blocks_meta_element["element_second_level"]]

                                blocks_meta_element["block_elements"].append({enum_key: enum_value})
                        except:
                            pass
            except:
                pass
            ...
            blocks_missing_enums_info = [
                {
                    ...
                    "block_elements": [
                        dict(t)
                        for t in {tuple(d.items()) for d in blocks_meta_element["block_elements"]}
                    ],
                }
                for blocks_meta_element in blocks_meta_info
                if len(blocks_meta_element["block_elements"]) > 0
            ]
